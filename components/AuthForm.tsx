...
const AuthForm = ({type} :{type: string}) => {
    ...
    const formSchecma = authFormSchema(type)

    const form = useForm<z.infer<typeof formSchecma>>({
        resolver: zodResolver(formSchecma),
        defaultValues: {
        email: "",
        password: "",
        },
    ...
    const onSubmit = async (data: z.infer<typeof formSchecma>) => {
        ...
        try{
            console.log(data)
            TODO: //SIGNUP with appwrite and create plaid token
            if (type === 'sign-up') {
        ...
        <header className='flex flex-col gap-5 md:gap-8'>
            ...
            <div className="flex flex-col gap-1 md:gap-3">
                ...
                <p className="text-16 font-normal text-gray-600">
                    {user ? 'Link your bank account to get started' : 'Please enter your details'}
                </p>
            </div>
        </header>
        {user ? (
